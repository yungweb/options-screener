...
# FIX 1 + 2: prepost=True added, time gate removed
...
def calc_trade(entry, stop, target, direction, days_to_exp, account, risk_pct, current_price, iv=0.45, atr=None, trade_style="swing"):
    ...
    # FIX 4: stop sanity — use ATR-based stop, never a random small number
    atr_safe = atr if (atr and atr > 0 and atr > current_price * 0.005) else current_price * 0.015
    if is_call:
        sane_stop = round(current_price - atr_safe * 0.5, 2)
    else:
        sane_stop = round(current_price + atr_safe * 0.5, 2)
    display_stop = sane_stop
    ...
    return {
        ...
        "target": round(stock_target, 2), "stop": display_stop, "entry": round(entry, 2),
...
def check_entry_confirmation(df, direction):
    ...
    if is_bull:
        ...
        if confirmed:   status = "CONFIRMED"; message = f"2 bullish candles confirmed. Entry window open near ${float(c2['close']):.2f}"
        elif last_green: status = "WAITING";  message = "1 of 2 bullish candles printed. Need 1 more green candle."
        else:            status = "AGAINST";  message = "Price still dropping. Wait for 2 consecutive green candles."
    else:
        ...
        if confirmed:  status = "CONFIRMED"; message = f"2 bearish candles confirmed. Entry window open near ${float(c2['close']):.2f}"
        elif last_red:  status = "WAITING";  message = "1 of 2 bearish candles printed. Need 1 more red candle."
        else:           status = "AGAINST";  message = "Price still climbing. Wait for 2 consecutive red candles."
...
def add_to_watch_queue(ticker, direction, sig, opt):
    ...
    if key not in st.session_state.watch_queue:
        st.session_state.watch_queue[key] = {
            ...
            "strike": opt["strike"], "entry": opt["entry"],
            "target": opt["target"], "stop": opt["stop"],
            "pattern": sig["pattern_label"], "confidence": sig["confidence"],
            "added_at": datetime.now(), "last_checked": None,
            "status": "WAITING", "message": "Watching for 2 confirmation candles...", "alerted": False,
        }
...
def run_background_watch_checks(tf_mult, tf_span, tf_days):
    ...
    for key, item in queue.items():
        ...
        try:
            ...
            if conf["confirmed"] and not was_confirmed and not item["alerted"]:
                item["alerted"] = True
...
def detect_market_regime(df):
    ...
    directional   = max(bull_bars, 10 - bull_bars)
    consistency_score = int((directional - 5) / 5 * 100)
    if atr_expanding and directional >= 7:
        regime = "trending"; strength = min(100, int(consistency_score * 1.2))
    elif not atr_expanding and directional <= 6:
        regime = "choppy";   strength = max(0, 100 - consistency_score)
    else:
        regime = "trending" if directional >= 7 else "choppy"; strength = consistency_score
...
def check_liquidity(ticker):
    try:
        ...
        return liquid, avg_vol, avg_oi, f"Avg vol {avg_vol:.0f} | OI {avg_oi:.0f}"
    except:
...
# FIX 4: calc_quick_levels with proper ATR fallback — no more $31 stops on $118 stocks
...
def fetch_multi_tf(ticker, trade_style):
    ...
    def _fetch(ticker, interval, period):
        try:
            # FIX 2: prepost=True for pre-market data
            df = yf.download(ticker, period=period, interval=interval,
                             progress=False, auto_adjust=True, prepost=True)
...
def check_vwap_confluence(df_5min, direction):
    ...
    if is_bull:
        ...
        if reclaim:  label = f"5min VWAP reclaimed ↑ (${vwap:.2f}) — strong"
        elif holding: label = f"5min holding above VWAP (${vwap:.2f})"
        else:         label = f"5min below VWAP (${vwap:.2f}) — no reclaim yet"
    else:
        rejection = prev > vwap and price < vwap
        holding   = prev < vwap and price < vwap
        passes    = rejection or holding
        if rejection: label = f"5min VWAP rejected ↓ (${vwap:.2f}) — strong"
        elif holding: label = f"5min holding below VWAP (${vwap:.2f})"
        else:         label = f"5min above VWAP (${vwap:.2f}) — no rejection yet"
    return passes, label
...
# FIX 3: Series-to-float crash fix
def check_tf_trend_agreement(dfs, direction):
    ...
    for label, df in dfs.items():
        ...
        try:
            ...
            details.append({"tf": label, "trend": trend, "agrees": agrees,
                            "ema20": round(ema20_val, 2), "price": round(price_val, 2)})
        except Exception:
            continue
    return agreeing, len(details), details

def build_multi_tf_candidates(ticker, toggles, account, risk_pct, dte, trade_style, atr=None):
    ...
    cands = build_candidates(primary_df, ticker, toggles, account, risk_pct, dte, trade_style=trade_style, atr=atr)
    ...
    for c in cands:
        ...
        if trade_style == "quick":
            ...
            extra_pass, extra_label = check_ema50_slope(tfs.get("daily"), direction)
            extra_name = "Daily EMA50"

        factor_pts   = c.get("score", 0)
        score_50_100 = (
            50
            + int((factor_pts / 5) * 25)
        ...
        c["tf_total"]        = tf_total
        c["extra_confluence"] = {"name": extra_name, "pass": extra_pass, "label": extra_label}
...
def log_trade(ticker, sig, opt, gates_passed, gates_total, elevate):
    ...
    journal.append({
        "Date": datetime.now().strftime("%m/%d %H:%M"), "Ticker": ticker,
        "Action": "CALL" if sig["direction"] == "bullish" else "PUT",
        "Pattern": sig["pattern_label"], "Strike": f"${opt['strike']:.2f}",
        "Entry": f"${opt['entry']:.2f}", "Target": f"${opt['target']:.2f}",
        "Stop": f"${opt['stop']:.2f}", "Premium": f"${opt['premium']:.2f}",
        "Contracts": opt["contracts"], "Max Loss": f"${opt['max_loss']:.0f}",
        "Pot. Profit": f"${opt['profit_at_target']:,.0f}", "Confidence": f"{sig['confidence']}%",
        "Gate Score": f"{gates_passed}/{gates_total}", "Elevated": "YES" if elevate else "no",
        "Expiry": opt["expiration"], "Result": "Open", "P&L $": "",
    })
...
def build_share_text(ticker, sig, opt, gates_passed, gates_total, elevate, market_status):
    ...
    sep = "=" * 32
    return (f"OPTIONS SCREENER v6.1 SIGNAL\n{sep}\n"
    ...
    for i, sig in enumerate(candidates):
        ...
        if sig["confidence"] >= 60:
            ...
            gate_html = (
                ...
                + elev_badge +
                "</div>" + gates_dots +
                "<div style='color:#8899aa;font-size:0.75rem;margin-top:6px'>Pattern needs ~" + str(est_days) + " days | Recommended DTE: " + str(dte_rec) + "+ days</div>"
                "</div>"
            )
            ...
            if htf_trend is not None:
                ...
                htf_detail = "Daily chart agrees — higher timeframe aligned." if htf_agrees else "Daily chart is moving the other way. Extra caution — counter-trend trade."
                htf_html = (
                    ...
                    "<span style='color:#8899aa;font-size:0.78rem;margin-left:4px'>Daily: " + htf_trend.upper() + " | RSI " + str(htf_rsi) + " | EMA20 $" + str(htf_ema) + "</span>"
                    "</div>"
            ...
            move_html = (
                ...
                "</div>"
                "<div style='margin-top:4px;font-size:0.82rem'>Price needs to move <b style='color:#e0e6f0'>" + str(opt["move_pct"]) + "%</b>"
            ...
            if sig_style == "quick":
                ...
                exit_take = f"Take 50% off when option reaches ${opt['exit_take_half']:.2f}/sh (100% gain). Let remaining 50% run."
            ...
            <div class='exit-rules'>
                ...
                <div style='margin:4px 0'><b>Close 100%</b> if {ticker} closes {'below' if is_bull else 'above'} <b style='color:#ff4d6d'>${opt['exit_stop_stock']:.2f}</b></div>
            ...
            if mstatus == "open":
                ...
                st.markdown("<div style='background:#0d1219;border:1px solid #1e2d40;border-radius:8px;padding:10px 14px;margin-top:8px;color:#8899aa;font-size:0.82rem'>⏸ Entry timing check runs during market hours only</div>", unsafe_allow_html=True)
            ...
            if mstatus == "open":
                ...
                st.markdown("<div style='background:#0d1219;border:1px solid #1e2d40;border-radius:8px;padding:10px 14px;margin-top:8px;color:#8899aa;font-size:0.82rem'>🤖 AI Trade Brief available during market hours</div>", unsafe_allow_html=True)
...
def get_market_internals():
    try:
        ...
        results = {}
        for sym in ["SPY", "QQQ"]:
            df = yf.download(sym, period="5d", interval="15m", progress=False, auto_adjust=True, prepost=True)
            ...
            price = float(close.iloc[-1])
            rsi   = calc_rsi(close)
            results[sym] = {"above_ema20": price > ema20, "above_ema50": price > ema50, "rsi": rsi}
...
def get_sector_bias(sector_etf):
    try:
        import yfinance as yf
        df = yf.download(sector_etf, period="5d", interval="1h", progress=False, auto_adjust=True, prepost=True)
...
# FIX 5: detect_exhaustion requires 2/4 and volume must be >= 1.2x avg
def detect_exhaustion(df, direction):
    ...
    # 1. Exhaustion candle — big body + volume MUST be >= 1.2x avg (was broken before)
    exh_found = False
    for j in range(-6, 0):
        ...
        if body > 0 and is_big and is_vol:
            score += 1
            reasons.append(f"{'Capitulation' if is_bull else 'Climax'} candle confirmed ({vol_ratio:.1f}x vol)")
            exh_found = True
            break
    if not exh_found:
    ...
    if is_bull:
        ...
        if is_hammer or is_doji:
            score += 1; reasons.append("Hammer/doji reversal candle")
        else:
        ...
        if is_star or is_doji:
            score += 1; reasons.append("Shooting star/doji reversal candle")
        else:
            reasons.append("No reversal candle yet")
    ...
    if div and ((is_bull and div.get("type") == "bullish") or (not is_bull and div.get("type") == "bearish")):
        score += 1; reasons.append("RSI divergence confirmed")
    else:
    ...
    if is_bull:
        ...
        if lows[-1] > lows[-2]:
            score += 1; reasons.append("Higher low structure forming")
        else:
            reasons.append("Lower low — structure not confirmed")
    else:
        highs = [float(high.iloc[i]) for i in [-15, -8, -1]]
        if highs[-1] < highs[-2]:
            score += 1; reasons.append("Lower high structure forming")
        else:
...
# FIX 6 + 7 + 8: gap check, sector momentum, signal invalidation, 4/5 threshold
def precision_score(ticker, direction, df_primary, df_confirm,
    ...
    try:
        ...
        if not _m.isnan(_cp) and _cp < 15:
            return None, f"Stock under $15 — options liquidity too thin ({_cp:.2f})"
    except Exception:
        pass

    # FIX 6: Pre-market / intraday gap check
    try:
        import yfinance as yf
        ...
        if prev_close > 0 and curr_price > 0:
            ...
            if direction == "bearish" and gap_pct > 3.0:
                return None, f"Gapped up {gap_pct:.1f}% against PUT signal — invalidated"
            if direction == "bullish" and gap_pct < -3.0:
                return None, f"Gapped down {gap_pct:.1f}% against CALL signal — invalidated"
    except Exception:
        pass

    # FIX 8a: Sector momentum hard stop
    try:
        import yfinance as yf
        ...
        if sector_etf and sector_etf != ticker:
            ...
            if sec_prev > 0 and sec_curr > 0:
                ...
                if direction == "bearish" and sec_move > 2.0:
                    return None, f"Sector {sector_etf} up {sec_move:.1f}% — fighting PUT signal"
                if direction == "bullish" and sec_move < -2.0:
                    return None, f"Sector {sector_etf} down {sec_move:.1f}% — fighting CALL signal"
    except Exception:
        pass

    # FIX 8b: Signal invalidation — price moved 1.5x ATR against signal
    try:
        if atr and atr > 0 and current_price and df_primary is not None and len(df_primary) >= 2:
            entry_price = float(df_primary["close"].iloc[-1])
            if direction == "bullish":
                recent_high = float(df_primary["high"].iloc[-10:].max())
                drop = recent_high - entry_price
                if drop > atr * 1.5:
                    return None, f"Price dropped {drop:.2f} ({drop/atr:.1f}x ATR) from recent high — CALL invalidated"
            else:
                recent_low = float(df_primary["low"].iloc[-10:].min())
                rip = entry_price - recent_low
                if rip > atr * 1.5:
                    return None, f"Price ripped {rip:.2f} ({rip/atr:.1f}x ATR) from recent low — PUT invalidated"
    except Exception:
        pass

    # ── TIER 2: Quality signals — need 4 of 5 (FIX 7) ────────────────────────
    signals_hit   = 0
    signal_detail = []
    ...
    try:
        ...
        if trend_dir == direction:
            signals_hit += 1; signal_detail.append("✅ Trend aligned")
        else:
    ...
    try:
        ...
        if vol_ratio >= 1.2:
            signals_hit += 1; signal_detail.append(f"✅ Volume confirming ({vol_ratio:.1f}x avg)")
        else:
            signal_detail.append(f"❌ Volume weak ({vol_ratio:.1f}x avg — need 1.2x+)")
    except Exception:
        signal_detail.append("❌ Volume unavailable")

    exh_confirmed, exh_score, exh_reasons = detect_exhaustion(df_primary, direction)
    if exh_confirmed:
        signals_hit += 1; signal_detail.append(f"✅ Exhaustion confirmed ({exh_score}/4 signals)")
    else:
        signal_detail.append(f"❌ Exhaustion weak ({exh_score}/4 — need 2+)")

    try:
        sq_state, sq_compression = detect_squeeze(df_primary, direction)
        if sq_state == "firing" and sq_compression >= 40:
            signals_hit += 1; signal_detail.append(f"✅ Squeeze FIRING — {sq_compression}% compressed")
        elif sq_state == "squeeze" and sq_compression >= 40:
            signals_hit += 1; signal_detail.append(f"✅ Squeeze ACTIVE — {sq_compression}% compressed")
        elif sq_state == "firing":
            signal_detail.append(f"❌ Weak squeeze exit ({sq_compression}% — not meaningful)")
        else:
            signal_detail.append("❌ No squeeze")
    except Exception:
    ...
    try:
        ...
        if div and ((direction == "bullish" and div.get("type") == "bullish") or
            ...
            signals_hit += 1; signal_detail.append("✅ RSI divergence")
        else:
    ...
    return final, {
        "exhaustion_confirmed": exh_confirmed, "exhaustion_score": exh_score,
        "exhaustion_reasons": exh_reasons, "signals_hit": signals_hit,
        "signal_detail": signal_detail, "tf_agree": tf_agree, "tf_total": tf_total,
        "market_bias": market_bias, "sector_bias": sector_bias,
        "liq_ok": liq_ok, "liq_vol": liq_vol,
    }
    ...
    try:
        ...
        for style, df_pri, df_con, dte in styles:
            ...
            conf, detail = precision_score(
                ...
                iv_rank, earn_days, market_bias, sec_bias,
            ...
            results.append({
                "ticker": ticker, "style": style, "direction": direction,
                "action": "CALL" if direction == "bullish" else "PUT",
                "pattern": best["pattern_label"], "confidence": conf,
                "gates_passed": gates_passed, "elevate": elevate,
                "entry_status": entry_status, "opt": opt, "sig": best,
                "price": round(cur_price, 2), "iv_rank": iv_rank,
                "earn_days": earn_days, "detail": detail,
                "market_bias": market_bias, "sector_bias": sec_bias,
                ...
                "rel_vol": rel_vol, "vol_spike": vol_spike,
...
def full_scan(scan_list, toggles, account_size, risk_pct,
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = {
            executor.submit(
                ...
                dte_quick, dte_swing, max_premium, trade_style_filter, market_bias
            ): ticker
    ...
    # FIX: use .get() to avoid KeyError on confidence
...
# ── Sidebar ───────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("## OPTIONS SCREENER v6.1")
    st.markdown("---")
    ...
    dte_quick = st.selectbox("Quick expiry", [0, 1, 2, 3, 5, 7], index=2, label_visibility="collapsed")
    st.markdown("**📅 Swing DTE** (multi-week)")
    dte_swing = st.selectbox("Swing expiry", [14, 21, 30, 45, 60], index=2, label_visibility="collapsed")
    trade_style = "both"
    st.markdown("---")
...
if any_new_confirm:
    st.markdown("""<audio autoplay><source src="data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAA" type="audio/wav"></audio>""", unsafe_allow_html=True)
            ...
            with col_banner:
                ...
                <div style='background:#061a10;border:2px solid #00d4aa;border-radius:10px;padding:14px 16px;margin:4px 0'>
                    ...
                    <div style='display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px;font-size:0.85rem;margin-top:6px'>
                        ...
                        <div><div style='color:#8899aa;font-size:0.7rem'>STOP</div><div style='font-weight:700;color:#ff4d6d'>${item["stop"]:.2f}</div></div>
                    </div>
                    <div style='margin-top:8px;color:#e0e6f0;font-size:0.78rem'>Candles: {candle_html}</div>
                </div>
                """, unsafe_allow_html=True)
        else:
            if status == "WAITING":
            ...
            with col_banner:
                ...
                <div style='{banner_style}'>
                    <div style='display:flex;justify-content:space-between;align-items:center'>
                        ...
                        <span style='color:#8899aa;font-size:0.82rem;margin-left:8px'>{item['pattern']} | Strike ${item['strike']:.2f}</span></div>
...
for ng in new_go_now:
    ...
    <div style='background:#061a10;border:2px solid #00e5aa;border-radius:10px;padding:14px 18px;margin:6px 0'>
        ...
        <div style='font-size:0.8rem;color:#8899aa;margin-top:2px'>{ng["pattern"]} · {ng.get("confidence",0)}% · {ng["gates_passed"]}/7 gates · Strike ${ng["opt"]["strike"]:.2f}</div>
    </div>
    """, unsafe_allow_html=True)
...
if earnings_days is not None:
    if earnings_days <= 1:   st.error(f"EARNINGS {'TODAY' if earnings_days==0 else 'TOMORROW'} on {selected_ticker}")
    elif earnings_days <= 7: st.error(f"EARNINGS IN {earnings_days} DAYS on {selected_ticker} - gate will block.")
    else:                    st.warning(f"Earnings in {earnings_days} days on {selected_ticker}")
...
with tab2:
    ...
    fig.add_trace(go.Bar(x=df["timestamp"], y=df["volume"], marker_color=vol_colors, opacity=0.5, name="Volume"), row=2, col=1)
    fig.update_layout(paper_bgcolor="#0a0e17", plot_bgcolor="#0d1219", font=dict(color="#e0e6f0", size=12),
        ...
        hovermode="x unified",
        modebar_remove=["pan", "lasso2d", "select2d", "autoScale2d", "toggleSpikelines", "zoomIn2d", "zoomOut2d"])
    fig.update_xaxes(gridcolor="#1e2d40", tickformat=tick_format, nticks=8, showspikes=True, spikecolor="#1e2d40")
    fig.update_yaxes(gridcolor="#1e2d40", tickformat="$.2f", dtick=tick_interval, range=[price_min, price_max], showspikes=True, row=1, col=1)
...
with tab4:
    ...
    if has_auto_results:
        ...
        st.caption(f"Showing results from {last_str} · {len(scan_list)} tickers scanned")
    ...
    if go_now or watching or on_deck:
        ...
        def mobile_card(r, bucket, idx):
            ...
            cc  = conf_color(r.get("confidence", 0))
            cl  = conf_label(r.get("confidence", 0))
            opt = r["opt"]
            ...
            dash   = round((r.get("confidence", 0) / 100) * circ, 1)
            ...
            parts = [
                ...
                "<div style='font-size:0.95rem;font-weight:700;color:%s;line-height:1'>%s</div>" % (cc, conf_val),
            ...
            with st.expander(f"📊 {r['ticker']} full details"):
                ...
                with c1:
                    for lbl, val, col in items_l:
                        st.markdown("<div style='background:#0d1421;border-radius:8px;padding:10px;margin-bottom:6px'><div style='font-size:0.58rem;color:#8899aa'>%s</div><div style='font-size:0.95rem;font-weight:700;color:%s'>%s</div></div>" % (lbl, col, val), unsafe_allow_html=True)
                with c2:
                ...
                if exh:
                    ...
                    for reason in exh:
                        good = any(x in reason for x in ["confirmed", "forming", "Higher low", "Lower high", "Climax", "Capitulation", "Hammer", "doji", "star"])
                        col  = "#00e5aa" if good else "#ff4d6d"
                        tcol = "#e0e6f0" if good else "#8899aa"
                        st.markdown("<div style='font-size:0.71rem;color:%s;padding:1px 0'><span style='color:%s'>●</span> %s</div>" % (tcol, col, reason), unsafe_allow_html=True)
...
with tab5:
    ...
    if stats:
        ...
        for t, v in sorted(stats.items(), key=lambda x: x[1]["total"], reverse=True):
            ...
            stat_rows.append({"Ticker": t, "Signals": v["total"], "Calls": v["calls"], "Puts": v["puts"],
                               "Open": v["open"], "Wins": v["wins"], "Losses": v["losses"], "Win Rate": wr})
    ...
    if not journal:
        ...
        with col_b:
            ...
            st.download_button("Export Journal CSV", data=csv,
                file_name=f"journal_{datetime.now().strftime('%m%d%Y')}.csv", mime="text/csv")
        with col_c:
            total  = len(journal)
...
st.markdown("<div style='text-align:center;padding:20px;color:#8899aa;font-size:0.75rem;border-top:1px solid #1e2d40;margin-top:20px'>OPTIONS SCREENER v6.1 - NOT FINANCIAL ADVICE</div>", unsafe_allow_html=True)
